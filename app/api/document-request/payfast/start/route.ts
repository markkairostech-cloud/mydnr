...
export async function POST(req: Request) {
  try {
    ...
    const m_payment_id = requestId;

    const return_url =
      `${siteUrl}/request-document/complete?requestId=${encodeURIComponent(
        requestId
      )}`;

    const cancel_url =
      `${siteUrl}/request-document`;

    const notify_url =
      `${siteUrl}/api/document-request/payfast/itn`;
    ...
    const fields: Record<string, string> = {
      ...
      name_first: requestorName,
      email_address: requestorEmail,

      m_payment_id,
      amount,

      item_name: "MyDNR Document Retrieval",
      item_description:
